import { JiraAuth, JiraIssue, JiraWorklog, JiraUser, JiraError } from '../types/jira.js';
...
export class JiraClient {
   ...
   */
  async addWorklog(issueKey: string, worklog: Omit<JiraWorklog, 'id' | 'author'>): Promise<JiraWorklog> {
   ...
   */
  async searchIssues(jql: string, maxResults: number = 10): Promise<{ issues: JiraIssue[]; total: number }> {
   ...
   */
  async getMyRecentIssues(maxResults: number = 5): Promise<JiraIssue[]> {
   ...
   */
  async getCurrentSprint(): Promise<{ id: number; name: string; startDate?: string; endDate?: string }> {
    ...
    const sprints = await sprintResponse.json() as { values: any[] };
   ...
   */
  async getSprintsForBoard(boardId: number): Promise<Array<{ id: number; name: string; state: string; startDate?: string; endDate?: string }>> {
    ...
    const sprints = await response.json() as { values: Array<{ id: number; name: string; state: string; startDate?: string; endDate?: string }>; isLast: boolean; maxResults: number; startAt: number; total: number };
    ...
    while (!sprints.isLast && allSprints.length < sprints.total) {
      ...
      const nextSprints = await nextResponse.json() as { values: Array<{ id: number; name: string; state: string; startDate?: string; endDate?: string }>; isLast: boolean; maxResults: number; startAt: number; total: number };
   ...
   */
  async searchWorklogsByDateRange(startDate: Date, endDate: Date): Promise<Array<{ issue: JiraIssue; worklog: JiraWorklog }>> {
    // Format dates for JQL
